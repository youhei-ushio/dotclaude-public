...
| `terminal-title.py` | PostToolUse / Stop | session 名（`/rename` や plan 承認の自動命名で設定）から iTerm2 のタブ/ウィンドウタイトルと badge を更新。Stop 時は完了アイコン + 通知も発火 | タブ/ウィンドウタイトル（OSC 1/2）は汎用。badge（OSC 1337）と通知（OSC 9）は iTerm2 専用で非対応端末では無視される |
...
> **iTerm2 ユーザー向け**: 1 行目に表示されるセッション名と `terminal-title.py` hook が iTerm2 タブ/ウィンドウ/badge に書き出すセッション名は同一の値（`/rename` コマンドまたは plan 承認時の自動命名で設定される `~/.claude/sessions/{pid}.json` の `name` フィールド）を参照している。
