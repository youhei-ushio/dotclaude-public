...
| `handoff` | 作業状態を `~/.claude/handoff/` の Markdown に保存し、後で読み込んで続きから再開（`save` / `load` / `list`）。セッション跨ぎの引き継ぎやタスク切り替えに使う。タスク名のデフォルトと `load` 後のセッション名設定に `/rename` 由来のセッション名（`~/.claude/sessions/`）を参照する |
