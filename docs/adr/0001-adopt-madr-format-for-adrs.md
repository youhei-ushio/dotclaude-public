...
- 「検討した選択肢」「決定」「結果（メリット/デメリット）」を構造的に書け、レビューしやすいこと
...
- Markdown 完結・ツール非依存で、本リポの symlink 配布運用と相性が良い
- 選択肢の列挙と評価が構造化され、判断の追跡性が高い
- 既存の MADR エコシステムに沿うため学習コストが低い

### デメリット

- Nygard 式より節が多く、軽微な決定には記述コストが相対的に高い
- 日本語ラベル等の adaptation により本家 MADR と完全一致ではない（差分は意図的）
