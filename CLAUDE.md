...
- 着手前に指示の目的を理解する
- 確定済みのナレッジ（プロジェクトの設計資料・ADR・過去の確定回答等）を着手前に参照し、そこに答えがある点は推測で埋めない
- 参照してもなお残る不明・曖昧・不整合な点（指示の字面と目的の乖離を含む）は、黙って推測・棚上げせず質問票としてユーザーに提示する（各設問に推奨解＋理由を添える）。うち判断や成果を左右する点は回答を得てから着手し、軽微な点は前提を明示して進む
