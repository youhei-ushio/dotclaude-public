...
**確定した質問＋回答を利用リポジトリの qa ディレクトリ（`docs/qa/` または `qa/`。永続・git 追跡）に残す**。
...
- **知識ストア（qa ディレクトリ）は利用リポに生やす**。書き込み先は実行時に決める——`docs/qa/` が
  あればそこ、無ければリポジトリルートの `qa/` を作る（Step 7）。既にある規約に合わせるのが原則で、
  スキルが新しい置き場を勝手に増やさない。
- **今回のスコープは qa 層のみ**。`original-docs/` / `knowledge/` / ADR の導入は利用リポ側の責務であり、このスキルは行わない。ただし各質問票は `反映先` 行でどの確定知に効くかを指し示す（Step 7）。
...
**利用リポジトリの qa ディレクトリに 1 ファイル書く**（HVE aqod 形式）。
会話コンテキストの記憶ではなくこのファイルを正として読むこと（記憶と食い違う場合はファイルが正）。

**出力先と命名は利用リポの既存規約に合わせる。** リポによって qa の置き場（`docs/qa/` に集約 / ルート直下）も
ファイル名の付け方も違う。ここを固定値にすると、導入済みのリポで規約から外れた 1 本だけが混ざる。

```bash
# 出力先と命名の手がかりを 1 コマンドで集める。
# Bash 呼び出しごとに別シェルになるため、判定と参照を分けない。
qa_dir=$([ -d docs/qa ] && echo docs/qa || echo qa)
mkdir -p "$qa_dir"
echo "qa_dir=$qa_dir"
echo "--- 既存ファイル（命名規約の手がかり） ---"
ls -1 "$qa_dir" 2>/dev/null | grep -v '^README\.md$' | head -10
echo "--- README の命名規約（あればこれが正） ---"
grep -A 5 -i '命名\|naming' "$qa_dir/README.md" 2>/dev/null | head -10
```

1. **出力先**: 上記のとおり `docs/qa/` があればそこ、無ければリポジトリルートの `qa/`（`mkdir -p` は冪等）。
2. **ファイル名**: 上の出力から次の優先順で決める。
   1. `<qa_dir>/README.md` に命名規約が書かれていればそれに従う（例: `QA-<topic>-<Issue番号>.md`）
   2. 規約の記述が無ければ**既存ファイルの命名に倣う**（2 本以上が同じ形なら、それが規約）
   3. 既存が 0 本で規約も無ければ `<slug>-questionnaire.md`（`<slug>` は `title`、無ければ `scope` の主題を kebab-case にした英字。例: `search-filter-spec`）
...
- 反映先: {この回答を差分マージする先。分かる範囲でパスを書く。例: `docs/knowledge/<file>.md` / ADR 起票。未定なら「未定（確定後に追記）」}
...
- **`反映先` は省略しない**（未定なら「未定」と書く）。ここが空だと「回答は出たが、どの確定知に
  効くのか誰も知らない」状態になり、qa が輪から外れて溜まるだけの記録になる。知識管理を導入した
  リポでは、この行が akm（knowledge 生成）の入口になる。
...
- 削除するのは**回答用の揮発 HTML と、その元データ JSON のみ**。Step 7 で書いた qa ファイルは永続の正として残す。
...
その入口が Step 7 の **`反映先`** 行——そこに書いたパスへ差分マージするのが次の作業になる。
