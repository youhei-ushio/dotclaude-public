...
`tmux-pane-awaiting.sh` が `-parallel-(\d+)` でディレクトリ名から N を抽出する
設計。命名を変える場合は hook 側の正規表現も合わせて書き換える。
...
## 通知の wiring（tmux ペインボーダー + 通知音）

並走中に「どの parallel が完了したか」「どれがユーザー応答待ちか」を区別する
仕組みは、同梱の 2 つの hook + ランチャで構成する:

- **`tmux-pane-awaiting.sh`**（視覚・tmux 限定）— `tmux-grid.sh` で 6 ペイン
  グリッドを立てて並走する場合、発火ペインのボーダー色で状態を示す
  （応答待ち=赤 / 完了=緑 / 作業中=無印）。各ペインの statusline は最下部に
  埋もれて「どのペインが呼んでいるか」が分かりにくいため、ボーダー側に出す。
  tmux 外では何もしない。
...
- 6 ペイングリッドの起動・ボーダー書式は `tmux-grid.sh` 側で定義する。
  `DIR_PREFIX` / `NAME_PREFIX` / `PARALLELS` を環境変数で自プロジェクトに
  合わせて上書きする（後述 Phase 4 参照）。
...
- Docker Desktop / Docker Engine が動く OS（本 skill の例は WSL2 + Docker
  Desktop を想定。Linux / macOS ネイティブでも動作する）
- ペイングリッド表示を使う場合は `tmux`（`tmux-grid.sh` / `tmux-pane-awaiting.sh`）
...
### Phase 4: tmux グリッドで一覧表示

並走中の視認性を上げるため、各 parallel を 1 つの tmux セッションの
3×2 グリッドで開く。同梱の `tmux-grid.sh` がこれを自動化する:

```bash
# DIR_PREFIX / NAME_PREFIX を自プロジェクトに合わせて起動
DIR_PREFIX=myapp-parallel NAME_PREFIX=myapp PARALLELS=6 ./tmux-grid.sh
```

- `~/repos/<DIR_PREFIX>-N` の各 clone ディレクトリでペインを開き、
  `claude --name <NAME_PREFIX>-N` を起動する
- 各ペインのボーダーが `tmux-pane-awaiting.sh` により状態を示す
...
# 3. tmux グリッドを使っている場合は該当ペインを閉じる
#    （または tmux-grid.sh の PARALLELS を調整して開き直す）
```
...
- `tmux-pane-awaiting.sh` hook + `tmux-grid.sh`（同梱）— 並走ペインのボーダーで応答待ち/完了を表示
- `notify-sound.sh` hook（同梱）— クロスプラットフォームな通知音
