...
if [ -f docs/temp/pr-body.md ]:
    ...
    if [ -f "$OWN_MARK_FILE" ] && [ "$(cat "$OWN_MARK_FILE" 2>/dev/null)" = "$N" ]:
        ...
        gh pr view "$N" --json body -q .body > docs/temp/pr-body.md
...
BROWSER_TEST_DONE = ...  # 経路 A (create-pr 経由): create-pr Step 3 で
                         # 初回ブラウザテストを実施したかを示すブール。
                         # 判定キーは「PR 本文に `## 動作確認スクリーン
                         # ショット` セクションがあるか」で代用する
                         # (gh pr view --json body で取得後 grep)。
...
if [ "$MODE" = "review-only" ]:
    ...
    REBASED_THIS_ITERATION=False   # review-only では Step 4 が skip され参照されない
                                    # が、共通変数のため Step 0.4 で init される想定
    if [ "$BEHIND" -gt 0 ]:
...
### N 巡目 (commit なし、escalate 中断 / 理由: base-conflict)
...
- **経路 A (create-pr → review-pr)**: create-pr Step 5 が **作成** →
