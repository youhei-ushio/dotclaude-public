---
name: review-pr
description: 指定 PR (引数なしならカレントブランチの PR) をセルフレビューする。Reviewer A / B + Fact-checker の 3 エージェント並列構成 (worktree 分離)。自分の PR は「指摘 0 件 (auto-fix = 0)」まで最大 5 巡で auto-fix モード、collaborator の PR は自動的に review-only モードで GitHub に summary review コメントを投稿 (フラグ --review-only / --fix で明示 override 可。--fix で他者 PR を上書きするには対象リポへの write 権限が必要)。「/review-pr 123」「PR レビューして」「指摘消して」「セルフレビュー」のような自然言語で起動。create-pr Step 6 からも内部呼び出しされる。
...
ESCALATE_REASON = None   # escalate 検出時に理由を保持 (例: "base-conflict",
                         ...
                         # Step 7 の最終報告で出力する
POSTED_TO_GITHUB = False # review-only モード Step 6.5.3 で gh pr review が
...
review-only モードの `docs/temp/pr${N}-review-comment.md` の所有権:

- **経路 C (review-only モード)**: review-pr Step 6.5.1 が **作成** (集約 markdown
  を新規 Write、`mkdir -p docs/temp` で親ディレクトリ存在も担保) → ユーザー承認後
  Step 6.5.3 で `gh pr review --comment --body-file` 投稿 → Step 8 が
...
### Step 6.5: GitHub に summary review コメントを投稿 (review-only モードのみ)
...
`docs/temp/pr${N}-review-comment.md` に書き出す (ファイル名の `${N}` は
...
集約 markdown の全文を表示してから、`AskUserQuestion` で確認。**question
文には必ず以下のガード文言を含める**: 「**投稿すると collaborator に通知が
飛び、PR レビュー履歴に永続記録されます (API での削除は技術的には可能ですが、
UI からは手動操作が必要で、いったん見た / 通知を受け取った相手の印象は
取り消せません)。本文に問題ないか最終確認をお願いします**」。
...
- **review-only モード**: 不可逆な GitHub 投稿 (Step 6.5.3 `gh pr review --comment`)
  に至るまで全コマンドで必須
...
- 主マーク別の finding 件数 (Question 振替分は元主マーク側からは **控除済み**):
  ...
  - **二重カウント方針**: Question 振替された finding は **元の主マーク側
    の件数からは控除し、Question 件数のみに計上** する (Step 6.5.1 markdown
    のセクション振分と整合)。`Must-fix M` の `M` は Must-fix section に実掲載
    された件数のみ、`(Question 振替: K 件、M には未計上)` は内訳開示のため
    別途記載 (= M に **含まれない** ことを明示)
- silent-reject 件数 + 内訳
- 6.5 の投稿結果 (投稿済み / 編集後投稿済み / 投稿せず終了 / 投稿失敗)
...
if [ "$MODE" = "review-only" ] && [ "$POSTED_TO_GITHUB" = "True" ]:
    rm -f "docs/temp/pr${N}-review-comment.md"
```
...
- **`gh pr review --comment` 投稿は不可逆**: collaborator に通知が飛ぶため、
  Step 6.5.2 のユーザー承認を必ず取る。承認なしに投稿してはならない
