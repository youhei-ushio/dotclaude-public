...
6. **証跡はコミットしない**: 検証は画面の目視確認までとし、スクリーンショットを
   `docs/images/` にコミットしたり PR 本文へ貼ったりしない (対応のたびに
   `docs/images/` が肥大化するため廃止)
...
- **スクリーンショットは PR に掲載しない**: `docs/images/` へのコミットや
  `?raw=true` 形式での引用は行わない（対応のたびに `docs/images/` が肥大化する
  ため廃止）。画面確認は Step 3 のブラウザテストで実施し、証跡はコミットしない
