...
#
#   レイアウト (PARALLELS=5 の例):
#     [1] [2] [3]    ← 上段 (myapp-parallel-1,2,3)
#     [4] [5] [ ]    ← 下段 (myapp-parallel-4,5 / 右下は空ターミナル)
#
#   使い方:
...
# ===== 設定（環境に合わせてここだけ調整） =====
SESSION="${1:-parallel}"
BASE="$HOME/repos"            # worktree の親ディレクトリ
DIR_PREFIX="myapp-parallel"   # worktree ディレクトリ = $BASE/$DIR_PREFIX-N
NAME_PREFIX="myapp"           # claude --name = $NAME_PREFIX-N
                              # ※ NAME_PREFIX / DIR_PREFIX は send-keys でペインの
                              #   シェルに送出されるため、英数字・ハイフン等のシェル
                              #   安全なトークンのみにすること（空白・特殊文字は不可）。
PARALLELS=5                   # Claude Code を起動する worktree 数 (1〜6)
